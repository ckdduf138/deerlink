...
const EMPTY_DRAFT: AnswerDraft = { nickname: "", answers: {}, currentQuestion: 0 };
...
export function RoomClient({ room }: { room: LobbyRoom }) {
  ...
  const handleStart = (nickname: string) => {
    saveDraft(draftKey, { ...draft, nickname });
    setMode("answer");
  };
  ...
  const canResume = answeredCount > 0 && draft.nickname.length > 0;
