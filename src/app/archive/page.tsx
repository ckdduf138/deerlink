...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const description = "디어링크 공개방에서 모인 밸런스게임 결과를 살펴보세요. 질문마다 사람들이 어떤 답을 골랐는지 익명 집계로 볼 수 있어요.";
  return {
    title: "밸런스게임 결과 모음",
    description,
      ...
      <main className="max-w-2xl mx-auto px-4 pt-24 pb-20">
        ...
        {rooms.length === 0 ? (
          <div className="rounded-2xl border border-stone-200 bg-white px-6 py-14 text-center">
            ...
            <Link
              href="/discover"
              className="mt-6 inline-flex min-h-11 items-center gap-2 rounded-xl bg-amber-700 px-5 text-sm font-semibold text-white transition-colors hover:bg-amber-800"
            >
              지금 열린 공개방 보기
            ...
            {rooms.map((room) => (
              <li key={room.id}>
                <Link
                  ...
                  className="group block rounded-2xl border border-stone-200 bg-white p-5 transition-colors hover:border-amber-300 hover:bg-amber-50/40"
                >
