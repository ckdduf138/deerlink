...
- **체험은 가짜 데모가 아니라 실제 공개방이 맡는다.** 2026-09에 히어로 오른쪽의 데모 질문(코드에 박힌 가짜 친구 넷)을 걷어내고, 히어로는 제목·본문·CTA만 남긴 뒤 바로 아래 `PublicRoomsSection`(지금 답이 모이는 질문)을 붙였다. 데모는 눌러도 데이터가 안 남고 바로 아래 공개방과 같은 체험을 두 번 시켰다. 데모나 div로 만든 가짜 제품 스크린샷을 되살리지 말 것.
- **Answer Lock은 히어로 본문 문장이 말한다.** 공개방은 Answer Lock을 건너뛰고 익명이라 차별점(친구 답은 내가 답해야 열린다)을 보여주지 못한다. 그 문장을 빼면 랜딩이 익명 투표 사이트(푸슝·PIKU 자리)로 읽힌다.
...
- **섹션마다 레이아웃 계열을 바꾼다.** 현재: 텍스트 비대칭 split(히어로) / 제목 행 + 카드 2개(공개방) / 좌 제목 + 우 아코디언(FAQ) / 중앙 마감(CTA). 같은 계열을 두 번 쓰지 말 것.
