...
- **공개방 카드는 `/discover`와 랜딩이 크기만 다른 `RoomCard` 컴포넌트 하나를 공유한다** (`size: "compact" | "standard" | "featured"`). 예전엔 두 화면이 서로 다른 컴포넌트라 내용 순서·유형별 미리보기·하단 문구가 다 달라서 같은 방인데 다른 카드로 보였다 — 따로 만들지 말 것. 항상 질문 제목이 먼저 오고(방 이름이 아니라 질문이 궁금증을 만든다), 이동 신호는 화살표 아이콘 하나뿐이다. "참여하기"/"답하기" 같은 문구를 다시 달지 말 것 — 카드 전체가 링크고 화살표가 이미 그 뜻이다.
...
- **`compact`(=`/discover`)만 밸런스 게임에 답이 있으면 실시간 비율 막대(`BalanceRatioBar`)를 보여준다.** `standard`·`featured`(랜딩)는 답이 있어도 반드시 두 선택지만 보여준다 — 결과부터 보이면 답할 이유가 없어진다는 원칙은 그대로다.
- **랜딩 카드는 1위(featured) + 2개 벤토다** (`LANDING_ROOM_COUNT = 3`). 첫 칸(현재 정렬의 1위)만 `sm:row-span-2`로 세로 두 칸을 차지하고 "카드 — 떠 있는 강조" 레시피(`shadow-lg shadow-amber-100/60`)를 쓴다. "1위"라는 라벨을 붙이지 말 것 — eyebrow 금지 규칙과 같은 이유로, 카드 크기 자체가 순위를 말해야 한다.
...
- **섹션마다 레이아웃 계열을 바꾼다.** 현재: 좌측 정렬 한 열(히어로) / 제목 + 정렬 메뉴 + 벤토(1+2, 공개방) / 좌 제목 + 우 아코디언(FAQ) / 중앙 마감(CTA). 같은 계열을 두 번 쓰지 말 것.
